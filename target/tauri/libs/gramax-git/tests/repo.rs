...
#[fixture]
fn repo(
  #[default(&sandbox())] sandbox: &TempDir,
  #[default("")] url: &str,
) -> impl Repository<DummyCreds> + RepoExt {
  if url.is_empty() {
...
fn init_new(sandbox: TempDir) -> Result {
  GitRepository::init(sandbox.path(), DummyCreds)?;
  assert!(sandbox.path().join(".git").exists());
  Ok(())
}

#[rstest]
fn branch_info(_sandbox: TempDir, #[with(&_sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn branch_info_by_name(_sandbox: TempDir, #[with(&_sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn create_branch(_sandbox: TempDir, #[with(&_sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn delete_branch(_sandbox: TempDir, #[with(&_sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn add_and_commit(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn commit_with_parents(_sandbox: TempDir, #[with(&_sandbox)] repo: impl Repository<DummyCreds>) -> Result {
  repo.new_branch("test")?;
  repo.new_branch("test2")?;

  repo.checkout("master", true)?;
  let oid = repo.commit_with_parents("message", vec!["test", "test2"])?;
  assert_eq!(repo.repo().find_commit(oid)?.parent_count(), 2);
  Ok(())
}

#[rstest]
fn no_modified_commit(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn file_history(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds> + RepoExt) -> Result {
...
#[rstest]
fn reset(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn status(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn fastforward_merge(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
...
#[rstest]
fn normal_merge_no_conflicts(
  sandbox: TempDir,
  #[with(&sandbox)] repo: impl Repository<DummyCreds>,
) -> Result {
...
#[rstest]
fn normal_merge_with_conflicts(
  sandbox: TempDir,
  #[with(&sandbox)] repo: impl Repository<DummyCreds>,
) -> Result {
  let path = sandbox.path();
...
#[rstest]
fn restore(sandbox: TempDir, #[with(&sandbox)] repo: impl Repository<DummyCreds>) -> Result {
  let path = sandbox.path();
...
#[rstest]
fn stash_with_confict(sandbox: TempDir, #[with(&sandbox)] mut repo: impl Repository<DummyCreds>) -> Result {
  let path = sandbox.path();
...
fn stash_parent(
  sandbox: TempDir,
  #[with(&sandbox)] mut repo: impl Repository<DummyCreds> + RepoExt,
) -> Result {
  let path = sandbox.path();
  let file = path.join("file");
  fs::write(file, "content")?;
