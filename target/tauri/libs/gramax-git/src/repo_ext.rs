...
pub trait RepoExt {
  ...
  fn parent_of(&self, oid: Oid) -> Result<Oid>;
}

impl<C: Creds> RepoExt for GitRepository<C> {
  fn get_content<P: AsRef<Path>>(&self, path: P, commit_oid: Option<Oid>) -> Result<String> {
  ...
  fn parent_of(&self, commit_oid: Oid) -> Result<Oid> {
    let oid = self.0.find_commit(commit_oid)?.parent(0)?.id();
    Ok(oid)
  }
