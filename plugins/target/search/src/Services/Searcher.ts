import { ArticleType, FileStatus, PArticle, PCatalog, PCatalogs, PChangeCatalog } from "@core/Plugin";
...
export default class ServicesSearcher implements Searcher {
	private _tag = "-query_";
	constructor(private _catalogs: PCatalogs, private _connection: Connection) {
		this._catalogs.onUpdate(this._getOnChangeRule().bind(this));
	}

	async resetAllCatalogs(): Promise<void> {
		let resetDataFlag = true;
		const catalogs: PCatalog[] = Array.from(await this._catalogs.getAll());
	...
	private async _getArticleData(article: PArticle, catalog: PCatalog): Promise<SaveServiceData> {
