...
struct WithRemote {
  local: Repo<DummyCreds>,
  local_path: PathBuf,
  remote: Repo<DummyCreds>,
  remote_path: PathBuf,
}
...
fn repos(#[default(&sandbox())] sandbox: &TempDir) -> WithRemote {
  ...
  let remote = Repo::init(&remote_path, DummyCreds).unwrap();
  remote.repo().config().unwrap().set_bool("core.bare", true).unwrap();
  let local =
    Repo::clone(remote_path.to_string_lossy(), &local_path, Some("master"), DummyCreds, |_, _| true).unwrap();

  WithRemote { local, local_path, remote, remote_path }
}

#[rstest]
fn with_no_remotes(_sandbox: TempDir, #[with(&_sandbox)] repo: Repo<DummyCreds>) -> Result {
  assert!(!(repo.has_remotes()?));
  Ok(())
}

#[rstest]
fn add_remote(_sandbox: TempDir, #[with(&_sandbox)] repo: Repo<DummyCreds>) -> Result {
...
fn clone(sandbox: TempDir, #[case] url: &str) -> Result {
  let mut objects = 0;
  Repo::clone(url, sandbox.path(), None, DummyCreds, |_, _| {
    objects += 1;
    true
  })?;
...
#[rstest]
fn auto_add_remote_postfix(_sandbox: TempDir, #[with(&_sandbox)] repo: Repo<DummyCreds>) -> Result {
