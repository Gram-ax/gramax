...
impl<C: Creds> Repo<C> {
  ...
  pub fn merge<S: AsRef<str>>(&self, theirs_branch: S) -> Result<()> {
  ...
  pub fn diff(&self, old: Oid, new: Oid) -> Result<StatusInfo> {
  ...
  pub fn status_file<P: AsRef<Path>>(&self, path: P) -> Result<crate::prelude::Status> {
    Ok(self.0.status_file(path.as_ref())?.into())
  }

  pub fn add_remote<S: AsRef<str>, U: AsRef<str>>(&self, name: S, url: U) -> Result<()> {
  ...
  pub fn branches(&self, branch_type: Option<BranchType>) -> Result<Branches> {
    Ok(self.0.branches(branch_type)?)
  }

  pub fn branch_by_name<S: AsRef<str>>(
    &self,
    shorthand: S,
  ...
  pub fn branch_by_head(&self) -> Result<BranchEntry<'_>> {
    let head = self.0.head()?;
    self.branch_by_name(head.shorthand().ok_or(Error::Utf8)?, BranchType::Local)
  }

  pub fn new_branch<S: AsRef<str>>(&self, shorthand: S) -> Result<BranchEntry<'_>> {
  ...
  pub fn delete_branch<S: AsRef<str>>(&self, shorthand: S, kind: BranchType) -> Result<()> {
    match kind {
      BranchType::Remote => {
        let mut cbs = RemoteCallbacks::new();
        cbs.credentials(make_credentials_callback(&self.1));
  ...
  pub fn stash(&mut self, message: Option<&str>) -> Result<Oid> {
  ...
  pub fn stash_apply(&mut self, oid: Oid) -> Result<()> {
  ...
  pub fn stash_delete(&mut self, oid: Oid) -> Result<()> {
...
impl<C: Creds> Repo<C> {
  ...
  fn ensure_remote_has_postfix(&self, remote: &'_ Remote) -> Result<()> {
