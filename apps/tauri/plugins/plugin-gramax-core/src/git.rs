...
#[command(async)]
pub(crate) fn storage_stats(_otel: OtelContext, repo_path: &Path) -> Result<StorageStats> {
	git::storage_stats(repo_path)
}
...
#[command(async)]
pub(crate) fn lfs_prune(_otel: OtelContext, repo_path: &Path) -> Result<usize> {
	git::lfs_prune(repo_path)
}
