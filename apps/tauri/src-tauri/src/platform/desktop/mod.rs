...
pub fn on_run_event<R: Runtime>(app: &AppHandle<R>, ev: RunEvent) {
	...
	match ev {
		...
		RunEvent::Reopen { .. } => {
			if let Some((_, window)) = app.webview_windows().iter().find(|(label, _)| label.starts_with("gramax-window")) {
				_ = window.show().or_show();
				_ = window.unminimize().or_show();
				std::thread::sleep(std::time::Duration::from_millis(300));
				_ = window.set_focus().or_show();
			} else {
				_ = MainWindowBuilder::default()
					.build(app)
...
fn assert_can_write<P: AsRef<Path>>(path: P) -> Result<()> {
	...
	let res = (|| {
		...
		if let Some(parent) = &parent {
			...
			if !std::fs::read_dir(parent)?.any(|p| p.as_ref().map(|p| p.file_name().as_os_str() == filename).unwrap_or(false)) {
