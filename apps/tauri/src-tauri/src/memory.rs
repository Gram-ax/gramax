...
pub enum ProcessMemoryInfo {
	...
	Webview {
		wv_label: String,
		process: GenericProcessMemoryInfo,
	},
	WebviewChild {
		parent_wv_label: String,
		process: GenericProcessMemoryInfo,
	},
}
...
impl MemoryInfo {
	...
	pub fn feed_webview<R: Runtime>(&mut self, webview: &WebviewWindow<R>, system: &System) -> Result<()> {
		...
		if let Some(process) = system.process(pid) {
			self.0.push(ProcessMemoryInfo::Webview {
				wv_label: webview.label().to_string(),
	...
	fn collect_child_processes(&mut self, parent_pid: Pid, system: &System) {
		for (pid, process) in system.processes().iter().filter(|(_, p)| p.parent().is_some_and(|p| p == parent_pid)) {
			self.0.push(ProcessMemoryInfo::Generic(process.into()));
			self.collect_child_processes(*pid, system);
		}
	}

	fn collect_webview_child_processes<R: Runtime>(&mut self, parent_pid: Pid, system: &System, webview: &WebviewWindow<R>) {
		for (pid, process) in system.processes().iter().filter(|(_, p)| p.parent().is_some_and(|p| p == parent_pid)) {
