...
async fn resolve_enterprise_version<R: Runtime>(app: &AppHandle<R>) -> std::result::Result<Option<semver::Version>, UpdaterError> {
	...
	let Ok(config) =
		serde_yml::from_str::<Config>(&std::fs::read_to_string(path)?).inspect_err(|e| warn!(target: TAG, "failed to parse config: {}", e))
	else {
		return Ok(None);
	};
