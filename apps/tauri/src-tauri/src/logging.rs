...
#[command]
pub fn js_log<R: Runtime>(window: WebviewWindow<R>, level: JsLogLevel, message: String, data: Vec<String>) -> Result<()> {
...
pub fn watch_process<R: Runtime>(app: AppHandle<R>) {
	std::thread::spawn(move || loop {
		...
		system.refresh_processes_specifics(ProcessesToUpdate::All, true, ProcessRefreshKind::nothing().with_cpu());
		...
		tracing::info!(target: "mem-usage", "{}", FmtProcessInfo(&memory_info));
