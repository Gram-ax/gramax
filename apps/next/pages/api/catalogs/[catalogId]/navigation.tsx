...
export default ApplyApiMiddleware(
	async function (req: ApiRequest, res: ApiResponse) {
		...
		const { catalog } = await dataProvider.getArticleByPathOfCatalog([req.query.catalogId as string]);

		const itemLinks = await dataProvider.getCatalogNav(catalog, "");
		const jsonNavigationTree = TransformData.getNavigation(itemLinks);
