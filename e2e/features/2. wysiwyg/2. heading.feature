# language: ru
Функция: Заголовки

  Сценарий: Превратить параграф в заголовок при помощи хоткея
    Пусть смотрим на редактор
    И заполняем документ
      """
      text(*)
      
      text
      """
    Когда нажимаем на клавиши "Control+Alt+2"
    Тогда разметка текущей статьи содержит
      """
      ## text
      
      text
      """

  Сценарий: Нажатие Enter внутри заголовка
    Пусть смотрим на редактор
    И заполняем документ
      """
      ## te(*)xt
      """
    И нажимаем на клавишу "Enter"
    Тогда разметка текущей статьи содержит
      """
      ## te
      
      xt
      """
    И очищаем документ

  Сценарий: Нажатие Enter вначале заголовка
    Пусть смотрим на редактор
    И заполняем документ
      """
      ## (*)text
      """
    Когда нажимаем на клавишу "Enter"
    Тогда разметка текущей статьи содержит
      """
      
      
      ## text
      """
    И очищаем документ

  Сценарий: Попытка создать заголовок в списке
    Пусть смотрим на редактор
    И заполняем документ
      """
      -  first
      
         -  second
      
            \##(*)
      """
    Когда нажимаем на клавишу "Space"
    И вводим "after"
    Тогда разметка текущей статьи содержит
      """
      -  first
      
         -  second
      
            \## after(*)
      """
    И очищаем документ

  Сценарий: Удаление заголовка через backspace и установка фокуса в предыдущий элемент
    Пусть смотрим на редактор
    И заполняем документ
      """
      another text
      
      ## text(*)
      """
    Когда нажимаем на клавиши "Control+Shift+ArrowLeft Backspace Backspace"
    И вводим "after"
    Тогда разметка текущей статьи содержит
      """
      another textafter
      """
    И очищаем документ
  # todo: пофиксить @NV
  # Сценарий: Переход из статьи в заголовок и обратно
  #   Пусть смотрим на редактор заголовка
  #   И вводим "Привет World"
  #   И нажимаем на клавишу "Enter"
  #   И ждём конца загрузки
  #   Тогда находимся по адресу "/-/-/-/-/new-catalog/privet-world"
  #   Когда нажимаем на клавишу "ArrowDown ArrowDown"
  #   И вводим "пока мир"
  #   Тогда разметка текущей статьи содержит
  #     """
  #     пока мир
  #     """
