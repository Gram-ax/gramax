...
import { Asset } from "@ext/workspace/assets/Asset";
...
export class PluginsAsset extends Asset {
	...
	async get(pluginId: string): Promise<PluginReadResult | null> {
		...
		const parsedMetadata = PluginFileParser.parseMetadata(metadata);
		const assets = await this._readStyleAssets(folder, PluginFileParser.getStylePaths(parsedMetadata));
	...
	async addFromConfig(plugin: PluginConfig): Promise<void> {
		...
		for (const asset of plugin.assets ?? []) {
			files.push({ path: PluginFileParser.getAssetStoragePath(asset), content: asset.content });
		}
	...
	private async _readStyleAssets(folder: Path, stylePaths: string[]): Promise<PluginAssetFile[]> {
		const assets: PluginAssetFile[] = [];

		for (const stylePath of stylePaths) {
			const storagePath = folder.join(new Path(PluginFileParser.getAssetStoragePath({ path: stylePath })));
			const content = await this._read(storagePath);
			assert(content != null, `Plugin style file missing: ${stylePath}`);
			assets.push({ path: stylePath, kind: "style", mimeType: "text/css", content });
		}
