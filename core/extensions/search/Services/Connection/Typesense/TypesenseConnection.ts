...
export default class TypesenseConnection implements Connection {
	...
	async resetData(data: SaveServiseData[]) {
		const collection = this._getCollection();
	...
	private _getCollection(): Collection<SaveServiseData> {
		return this._client.collections<SaveServiseData>(this._collectionName);
	}
