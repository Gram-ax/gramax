...
export default class FuseSearcher implements Searcher {
	...
	async search(query: string, catalogName: string, itemRefs: ItemRef[]): Promise<SearchItem[]> {
		...
		if (!itemRefs?.length || doesQueryHaveOnlyNegativeWords) return [];
		...
		const result: SearchItem[] = searchedArray
			.filter((searched) => itemRefs.some((itemRef) => itemRef.path.value === searched.item.path))
	...
	private _getName(searched: FuseResult<IndexData>, query: string) {
		const title = searched.item.title ?? "";
