...
class IsomorphicGitRepository implements Repository {
	...
	constructor(
		...
		private _parent: GitRepository,
		private _conf: GitRepositoryConfig,
	) {
	...
	async push(data: GitSourceData): Promise<void> {
		await git.push({
			...
			cache: IsomorphicGitRepository._cache,
			corsProxy: this._proxy,
		});
	}
	...
	async diff(oldTree: string, newTree: string): Promise<GitStatus[]> {
		return (
			await git.walk({
				fs: this._gitFs,
				dir: this._repoPath.value,
				trees: [git.TREE({ ref: oldTree }), git.TREE({ ref: newTree })],
	...
	async getFixedSubmodulePaths(): Promise<Path[]> {
		...
		return Promise.all(
			submodulesData.map(async (submoduleData): Promise<Path> => {
				...
				const subGitRepository = new GitRepository(this._conf, this._fp, submodulePath);
	...
	async getRemoteUrl(): Promise<string> {
		try {
			return await git.getConfig({
				fs: this._gitFs,
				dir: this._repoPath.value,
				path: `remote.${await this._getRemote()}.url`,
			});
		} catch {
			throw new Error(
	...
	private async _checkoutSubmodule(subGitRepository: GitRepository, branch: string) {
		try {
			if (branch) {
...
export default IsomorphicGitRepository;
