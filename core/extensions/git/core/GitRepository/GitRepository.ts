...
import { Caller } from "./errors/model/Caller";
import GitErrorCode from "./errors/model/GitErrorCode";

export class GitRepository {
	private _impl: Repository;

	constructor(conf: GitRepositoryConfig, private _fp: FileProvider, private _repoPath: Path) {
		const impl = resolveModule("GitRepositoryImpl");
	...
	async hasRemote(): Promise<boolean> {
		...
		const config = ini.parse(await this._fp.read(filePath));
	...
	async getHeadCommit(ref: GitBranch | string = "HEAD"): Promise<GitVersion> {
		return await this._logWrapper("getHeadCommit", `Getting head commit for ref:${ref.toString()}`, async () => {
			try {
				...
				this._log(`Getting head commit for ref:${ref.toString()} failed`, "getHeadCommit", e);
				throw getGitError(
					e,
	...
	async deleteBranch(branchName: GitBranch | string, remote?: boolean, data?: GitSourceData): Promise<void> {
		await this._logWrapper("deleteBranch", `Deleteing branch '${branchName}'`, async () => {
			const currentBranch = await this.getCurrentBranch();
			if (
				branchName.toString() === currentBranch.toString() ||
				branchName.toString() === currentBranch.getData().remoteName
			) {
				throw new GitError(GitErrorCode.DeleteCurrentBranch, null, { repositoryPath: this._repoPath.value });
			}
			try {
	...
	async clone(
		...
		return this._logWrapper("clone", `Cloning url: '${url}', path: '${this._repoPath}'`, async () => {
			...
			try {
				...
				await this._logWrapper("delete", `Deleting path: '${this._repoPath}'`, () =>
					this._fp.delete(this._repoPath),
				);
	...
	async pull(data: GitSourceData) {
		return await this._logWrapper("pull", "Pulling", async () => {
			...
			const remoteBranchName = new GitVersion(
			...
			await this.merge(data, remoteBranchName.toString());
	...
	private _getRemote(): Promise<string> {
		const REMOTE = "origin";
