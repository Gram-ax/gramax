...
import GitDiffItemCreatorLegacy from "../GitDiffItemCreatorLegacy";
...
const getGitDiffItemCreatorData = async () => {
	...
	const gitDiffItemCreator = new GitDiffItemCreatorLegacy(catalog, sitePresenter, fs);
	...
	describe("выдаёт DiffItems", () => {
		...
		test("с изменением ресурсов", async () => {
			...
			const res = data.items.map((x) => ({
				...
				resources: x.resources.map((r) => ({ path: r.filePath.path, title: r.title, type: r.status })),
			}));
		...
		test("с отдельными ресурсами", async () => {
			...
			const hasItemDiffs = data.items[0].hunks[0].type;
