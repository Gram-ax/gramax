...
class WordTemplate {
	...
	getTemplate(name: string) {
		return this._workspace.getAssets().wordTemplates.getContent(name);
	}
}
...
export class WordTemplateManager {
	...
	public static isWordTemplateName(templateName: string): boolean {
		...
		return WORD_TEMPLATE_FORMATS.some((v) => ext.startsWith(v));
	}
	...
	private async _getTemplates(workspace: Workspace) {
		...
		const templates = await workspace.getAssets().wordTemplates.list();
