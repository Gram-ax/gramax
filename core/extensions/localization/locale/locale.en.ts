import { DefaultLocaleType } from "@ext/localization/locale/translate";

const locale: DefaultLocaleType = {
	forms: {
		"catalog-edit-props": {
		...
		"gitlab-source-data": {
			props: {
				...
				token: {
					...
					placeholder: "glpat-aq6PK8sz1eQeKhTy-Dm5",
					description:
						"Token for reading and modifying repositories in storage. Specify the token permissions: api, read_repository, write_repository. <a target='_blank' href='https://docs.gitlab.com/ee/user/profile/personal_access_tokens.html'>Learn more</a>",
				},
				url: {
					name: "Git Server URL",
					placeholder: "https://git-server.com",
	...
	workspace: {
		...
		delete: {
			desktop: "Delete workspace? Catalogs will remain on your computer",
...
This is a private article. Log in with an account that has access or request permissions from the author.`,
			"500": {
				title: `---
...
Add storage to confirm the link.`,
	...
	share: {
		name: "Share",
		hint: "You can also simply copy the page URL",
		desc: "Make sure the recipient has access to the catalog. When they follow the link: if the catalog was not loaded — it will load, if the branch is not synchronized — Gramax will offer to sync it.",
		error: {
	...
	git: {
		...
		checkout: {
			...
			error: { "local-changes-present": "Your local changes prevent branch switching" },
		},
		branch: {
			error: {
		...
		merge: {
			...
			conflict: {
				"abort-confirm": {
					...
					body: {
						sync: "A conflict occurred during synchronization. You need to resolve it to complete synchronization. If you do not, the directory will return to its previous state.",
		...
		publish: {
			error: {
				"non-fast-forward": "Your catalog version is outdated. Sync it, then publish the changes",
				unknown: "Unknown error when publishing. Error message -",
				protected: "Branch protected from publishing",
				http: "HTTP Error: {{status}}",
				"no-permission": "You do not have permission to sync with this catalog",
			},
		},
		history: {
			name: "Change History",
	...
	confluence: {
		...
		error: {
			...
			"cannot-import": {
				title: "Failed to import “{{name}}” from Confluence",
	...
	"bug-report": {
		...
		error: {
			"cannot-send-feedback": "Unable to send bug report",
			"check-internet-or-adblocker": "Check your internet connection and disable ad blockers.",
		},
	},
	word: {
	...
	enterprise: {
		"incorrect-email": "The email provided is incorrect",
		"user-not-found":
	...
	"and-sync-catalog": "And sync catalog?",
	...
	"change-and-sync": "Change and sync",
	...
	"check-images": "Images",
	"check-links": "Links",
	"checkout-error": "Unable to switch branch",
	...
	"sync-catalog-desc": "The catalog version is outdated. To get changes, sync the catalog.",
	"sync-logs": "Sync logs",
