...
export default class CatalogEntryAttachGit implements EventHandlerCollection<FSEvents> {
	constructor(private _fs: FileStructure) {}

	mount({ events }: HasEvents<FSEvents>): void {
		events.on("before-catalog-entry-read", async ({ path, initProps }) => {
