...
export class Encoder {
	...
	public encode(data: string[], accessToken: string, encoding: "base64" | "hex" = "base64"): string {
		const iv = this._randomstring(16);
		const encryptedBase64 = this._encodeAes(data.join(this._dataChar), iv, accessToken, encoding);
	...
	public decode(accessToken: string, ticket: string, encoding: "base64" | "hex" = "base64"): string[] {
		...
		const data = this._decodeSafe(iv, encrypted, accessToken, encoding);
		if (!data) return null;
		return data.split(this._dataChar);
	}
	...
	private _encodeAes(utf8String: string, iv: string, accessToken: string, encoding: "base64" | "hex"): string {
