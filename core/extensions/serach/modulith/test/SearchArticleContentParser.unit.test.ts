...
describe("SearchArticleContentParser", () => {
	it("test testModel1", async () => {
		const actual = await new SearchArticleContentParser(testModel1.content, getSnippetItems, getPropertyValue, getLinkId).parse();
		expect(actual).toEqual(testModel1Expected);
	});
	
	it("test testModel2", async () => {
		const actual = await new SearchArticleContentParser(testModel2.content, getSnippetItems, getPropertyValue, getLinkId).parse();
		expect(actual).toEqual(testModel2Expected);
	});
	
	it("test testModel3", async () => {
		const actual = await new SearchArticleContentParser(testModel3.content, getSnippetItems, getPropertyValue, getLinkId).parse();
