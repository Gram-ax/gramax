...
// Bun workers cannot load Node-API modules via ESM `import`.
type Native = typeof import("../../../apps/next/crates/next-gramax-core");
const native = createRequire(import.meta.url)(resolve(process.cwd(), "../../apps/next/crates/next-gramax-core")) as {
	default: Native;
};
...
const callFs = async <O>(cmd: string, args: Record<string, unknown>): Promise<O> => {
	...
	const fn = (native.default as any)[cmd];
	...
	try {
		...
		const error = typeof err === "string" ? err : (err as Error)?.stack ? tryParse((err as Error).message) : err;
		if (error?.name === "Git") {
