...
const markAsOpened: Command<{ logicPath: string; catalogName: string; ctx: Context }, void> = Command.create({
	path: "article/markAsOpened",
	...
	async do({ ctx, logicPath, catalogName }) {
		...
		await es.markAsOpened({ user: userData, article: articleData, time: date, catalog: catalogData });
	},
