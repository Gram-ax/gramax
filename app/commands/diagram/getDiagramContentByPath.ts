...
const getDiagramContentByPath: Command<
...
> = Command.create({
	...
	async do({ ctx, src, articlePath, catalogName, type, count }) {
		...
		const hash: HashResourceManager = new HashResourceManager(src, fp, article.parsedContent.resourceManager);
...
export default getDiagramContentByPath;
