...
const get: Command<{ ctx: Context; catalogName: string; articlePath: string }, Link[]> = Command.create({
	path: "catalog/links/backlinks/get",
	...
	async do({ catalogName, articlePath, ctx }) {
		...
		const backlinks = await provider.getArticleBacklinks(articlePath);
		const formattedBacklinks = await provider.getFormattedLinks(backlinks);

		return formattedBacklinks;
	},
