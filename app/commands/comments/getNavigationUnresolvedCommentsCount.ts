...
const countComments = (editTree: JSONContent): number => {
	let count = 0;
	if (editTree.type == "text" && editTree?.marks?.find?.((m) => m.type === "comment")) count++;
	if (editTree.content) editTree.content.forEach((child) => (count += countComments(child)));
	return count;
};
