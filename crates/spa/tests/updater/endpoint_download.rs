...
async fn prepare_linux_deb(s3: &S3Client, version: &str) -> anyhow::Result<()> {
	...
	s3.put(&format!("dev/{part1}/{part2}/linux-x86_64/gramax.linux-x86_64.deb"), "deb-bytes")
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/linux-x86_64/gramax.linux-x86_64.deb.sig"), "deb-bytes")
		.await?;
	Ok(())
}
...
pub async fn download_linux_deb_with_package_param() -> anyhow::Result<()> {
	...
	let req = make_req("/linux-x86_64?channel=dev&package=deb")
		.method(Method::GET)
		.body(Body::empty())
		.unwrap();
	let res = app.call(req).await.unwrap();
