...
static mut MINIO_CONTAINER: tokio::sync::OnceCell<ContainerAsync<GenericImage>> =
...
async fn get_or_create_minio_address() -> String {
  let minio_arc = unsafe {
    ...
    MINIO_CONTAINER.get_or_init(|| async { create_minio().await.expect("failed to create minio") }).await
  };
...
pub async fn create_minio() -> anyhow::Result<ContainerAsync<GenericImage>> {
  ...
  let data_dir =
    MINIO_DATA_DIR.get_or_init(|| async { TempDir::new("minio-data").unwrap() }).await.path().to_path_buf();
...
pub fn updater(s3_base_url: S3BaseUrl) -> Router {
  Updater { s3_base_url: s3_base_url.0, metrics: Metrics::default() }.into_router()
}
