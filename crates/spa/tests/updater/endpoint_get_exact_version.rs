...
async fn prepare_darwin_aarch64(s3: &S3Client, version: semver::Version) -> anyhow::Result<()> {
	...
	s3.put(&format!("dev/{part1}/latest/gramax.darwin-aarch64.dmg.version"), version.to_string())
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/darwin-aarch64/gramax.darwin-aarch64.dmg"), "app")
		.await?;
	s3.put(
		&format!("dev/{part1}/{part2}/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz"),
		"app update",
	)
	.await?;
	s3.put(
		&format!("dev/{part1}/{part2}/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz.sig"),
		"sig",
	)
	.await?;
	Ok(())
...
async fn prepare_linux_deb(s3: &S3Client, version: semver::Version) -> anyhow::Result<()> {
	...
	s3.put(&format!("dev/{part1}/latest/gramax.linux-x86_64.deb.version"), version.to_string())
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/linux-x86_64/gramax.linux-x86_64.deb"), "app")
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/linux-x86_64/gramax.linux-x86_64.deb.sig"), "sig")
		.await?;
	Ok(())
}
...
async fn prepare_windows_nsis(s3: &S3Client, version: semver::Version) -> anyhow::Result<()> {
	...
	s3.put(&format!("dev/{part1}/latest/gramax.windows-x86_64.nsis.version"), version.to_string())
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/windows-x86_64/gramax.windows-x86_64.setup.exe"), "app")
		.await?;
	s3.put(&format!("dev/{part1}/{part2}/windows-x86_64/gramax.windows-x86_64.setup.exe.sig"), "sig")
		.await?;
	Ok(())
}
	...
	assert2::check!(
		update.url
			== s3
				.base_url()
				.await
				.with_path(["dev", "2.0", "0", "darwin-aarch64", "gramax.darwin-aarch64.update.tar.gz",])
	);
	...
	assert2::check!(
		update.url
			== s3
				.base_url()
				.await
				.with_path(["dev", "1.5", "0", "linux-x86_64", "gramax.linux-x86_64.deb"])
	);
	...
	assert2::check!(
		update.url
			== s3
				.base_url()
				.await
				.with_path(["dev", "1.2", "3", "windows-x86_64", "gramax.windows-x86_64.setup.exe"])
	);
	...
	assert2::check!(
		update.url
			== s3
				.base_url()
				.await
				.with_path(["dev", "1.0", "0", "darwin-aarch64", "gramax.darwin-aarch64.update.tar.gz",])
	);
	...
	assert2::check!(
		update.url
			== s3
				.base_url()
				.await
				.with_path(["dev", "1.0", "0", "linux-x86_64", "gramax.linux-x86_64.deb"])
	);
...
pub async fn get_exact_version_missing_signature_returns_404() -> anyhow::Result<()> {
	...
	s3.put("dev/1.0/0/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz", "app update")
		.await?;
