...
async fn prepare_artifacts(s3: &S3Client, versions: [&str; 4]) -> anyhow::Result<()> {
  let [latest_darwin_aarch64, latest_darwin_x86_64, latest_linux_x86_64, latest_windows_x86_64_nsis] =
    versions;

  let [url_darwin_aarch64, url_darwin_x86_64, url_linux_x86_64, url_windows_x86_64_nsis] =
    versions.map(|v| semver::Version::parse(v).unwrap().split()).map(|(p1, p2)| format!("{p1}/{p2}"));

  s3.put("dev/latest/gramax.darwin-aarch64.dmg.version", latest_darwin_aarch64.to_string()).await?;
  s3.put(&format!("dev/{url_darwin_aarch64}/darwin-aarch64/gramax.darwin-aarch64.dmg"), "app").await?;
  s3.put(
    &format!("dev/{url_darwin_aarch64}/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz"),
    "app update",
  )
  .await?;
  s3.put(&format!("dev/{url_darwin_aarch64}/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz.sig"), "sig")
    .await?;

  s3.put("dev/latest/gramax.darwin-x86_64.dmg.version", latest_darwin_x86_64.to_string()).await?;
  s3.put(&format!("dev/{url_darwin_x86_64}/darwin-x86_64/gramax.darwin-x86_64.dmg"), "app").await?;
  s3.put(&format!("dev/{url_darwin_x86_64}/darwin-x86_64/gramax.darwin-x86_64.update.tar.gz"), "app update")
    .await?;
  s3.put(&format!("dev/{url_darwin_x86_64}/darwin-x86_64/gramax.darwin-x86_64.update.tar.gz.sig"), "sig")
    .await?;

  s3.put("dev/latest/gramax.linux-x86_64.appimage.version", latest_linux_x86_64.to_string()).await?;
  s3.put(&format!("dev/{url_linux_x86_64}/linux-x86_64/gramax.linux-x86_64.appimage"), "app").await?;
  s3.put(&format!("dev/{url_linux_x86_64}/linux-x86_64/gramax.linux-x86_64.appimage.sig"), "sig").await?;

  s3.put("dev/latest/gramax.windows-x86_64.nsis.version", latest_windows_x86_64_nsis.to_string()).await?;
  s3.put(&format!("dev/{url_windows_x86_64_nsis}/windows-x86_64/gramax.windows-x86_64.setup.exe"), "app")
    .await?;
  s3.put(&format!("dev/{url_windows_x86_64_nsis}/windows-x86_64/gramax.windows-x86_64.setup.exe.sig"), "sig")
    .await?;
