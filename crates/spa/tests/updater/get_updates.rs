...
pub async fn get_latest_update_basic_test() -> anyhow::Result<()> {
	...
	s3.put("dev/1.1/1/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz", "app update")
		.await?;
	...
	s3.put("dev/2.2/2/darwin-aarch64/gramax.darwin-aarch64.update.tar.gz", "app update2")
		.await?;
