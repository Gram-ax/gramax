...
async fn main() {
	...
	let metrics = Metrics {
		cookie_domain: Arc::new(opts.cookie_domain),
		sender: metrics_sender.clone(),
	};

	let router = Router::new();

	let router = match opts.mode {
		Mode::Static { dir, fallback } => router.merge(static_assets::serve_static_assets(dir, fallback, metrics.clone())),
	...
	let server = axum::serve(listener, router.into_make_service_with_connect_info::<SocketAddr>()).with_graceful_shutdown(on_shutdown());

	server.await.unwrap();
}

async fn set_xreal_ip_if_none(ConnectInfo(addr): ConnectInfo<SocketAddr>, mut req: Request, next: Next) -> Response {
...
async fn init_metrics(opts: metrics::exporter::ElasticOptions) -> Result<metrics::exporter::MetricSender<MetricDoc>, anyhow::Error> {
