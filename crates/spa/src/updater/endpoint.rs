...
impl Updater {
  pub fn into_router(self) -> Router {
    ...
    let all_updates_check_middleware = ServiceBuilder::new()
      .layer(middleware::from_fn(crate::metrics::layers::insert_metrics_user_action_check_updates));

    let update_check_middleware = ServiceBuilder::new()
      .layer(middleware::from_fn(crate::metrics::layers::insert_metrics_user_action_check_single_update));
...
async fn get_all_updates(
  ...
  channel.update_latest_if_needed(None).await.report_if_err().map_err(UpdaterError::FailedToFetchUpdate)?;
  ...
  for platform in platforms {
    ...
    updates.insert(platform, PlatformInfo { url: update.s3_url.to_string(), signature: update.signature });
  }
...
async fn get_update(
  ...
  let update = match desired_app_version {
    Some(desired_version) => {
      ...
      channel.get_latest_update_by_version(desired_version.clone(), platform).await.report_if_err().map_err(
        |_| {
