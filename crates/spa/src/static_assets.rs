...
fn static_prepare_env() -> Result<Vec<u8>, std::io::Error> {
	...
	if !generated.status.success() {
		error!("failed to generate env.js: {}", String::from_utf8_lossy(&generated.stderr));
		return Err(std::io::Error::other("failed to generate env.js"));
	}

	debug!("generated env.js:\n{}", String::from_utf8_lossy(&generated.stderr));
	Ok(generated.stderr)
}

async fn set_headers(req: Request, next: Next) -> Response {
	let must_revalidate_cache = req.uri().path().matches('/').take(2).count() <= 1 && req.uri().path().matches("favicon.ico").next().is_none();
	...
	if must_revalidate_cache {
		let val = HeaderValue::from_static("no-cache, must-revalidate, max-age=0");
		headers.insert("cache-control", val);
		headers.insert("Vary", HeaderValue::from_static("*"));
	} else {
		headers.insert("cache-control", HeaderValue::from_static("public, max-age=2592000, immutable"));
	}
