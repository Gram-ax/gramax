...
impl BugsnagNotificationBuilder {
	...
	pub fn custom(&self, context: String, where_occured: String, message: Option<String>) -> BugsnagNotification {
		let exception = BugsnagException::new(where_occured);
		let exception = if let Some(message) = message {
			exception.with_message(message)
		} else {
			exception
		};
