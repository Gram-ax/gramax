...
pub fn add_resolve_rename(sandbox: TempDir, #[with(&sandbox)] repo: Repo<TestCreds>) -> Result {
  ...
  let status = repo.status(true)?.short_info()?;
  let expect = vec![
    StatusInfoEntry { path: PathBuf::from("dir/file"), status: StatusEntry::Delete },
    StatusInfoEntry { path: PathBuf::from("dir/file2"), status: StatusEntry::Delete },
    StatusInfoEntry { path: PathBuf::from("dir_2/file"), status: StatusEntry::New },
    StatusInfoEntry { path: PathBuf::from("dir_2/file2"), status: StatusEntry::New },
  ];
