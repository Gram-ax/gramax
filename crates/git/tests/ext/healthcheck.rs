...
fn healthcheck_missing_tree(_sandbox: TempDir, #[with(&_sandbox)] mut repo: Repo<TestCreds>) -> Result {
	...
	assert!(
		found_bad_commit.is_some(),
		"commit {commit_id} should be corrupted; found: {bad_objects:?}"
	);
...
fn healthcheck_missing_commit(_sandbox: TempDir, #[with(&_sandbox)] mut repo: Repo<TestCreds>) -> Result {
	...
	let found_missing_parent = bad_objects.iter().find(|obj| matches!(obj.reason, BadObjectReason::MissingParent { .. }));
...
fn healthcheck_with_refs_and_index(_sandbox: TempDir, #[with(&_sandbox)] repo: Repo<TestCreds>) -> Result {
	...
	assert_eq!(
		bad_objects.len(),
		0,
		"healthy repository after switching branches should not have problems"
	);
