...
fn read_tag(sandbox: TempDir, #[with(&sandbox)] repo: Repo<TestCreds>) -> Result {
	...
	repo
		.repo()
		.tag("tag", &repo.repo().find_object(oid, None)?, &DummyCreds.signature()?, "commit", true)?;
...
fn read_from_reference(sandbox: TempDir, #[with(&sandbox)] repo: Repo<TestCreds>) -> Result {
	...
	repo
		.repo()
		.tag("q", &repo.repo().find_object(oid, None)?, &TestCreds.signature()?, "commit", true)?;
...
#[rstest]
fn read_dir_stats(sandbox: TempDir, #[with(&sandbox)] repo: Repo<TestCreds>) -> Result {
	fs::create_dir_all(sandbox.path().join("dir/dir"))?;
	fs::write(sandbox.path().join("dir/file"), "content")?;
	fs::write(sandbox.path().join("dir/dir/file"), "content2")?;
	repo.add_all()?;
	...
	assert_eq!(stats.next().unwrap().name, "dir");
	assert_eq!(stats.next().unwrap().name, "file");
