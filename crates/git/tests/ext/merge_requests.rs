...
fn create_merge_request_same_branch(_sandbox: TempDir, #[with(&_sandbox)] repos: Repos) -> Result {
	...
	match err {
		gramaxgit::error::Error::Git(err) if err.code() == git2::ErrorCode::Invalid && err.class() == git2::ErrorClass::Reference => {}
...
fn create_merge_request_invalid_branch(_sandbox: TempDir, #[with(&_sandbox)] repos: Repos) -> Result {
	...
	match err {
		gramaxgit::error::Error::Git(err) if err.code() == git2::ErrorCode::NotFound && err.class() == git2::ErrorClass::Reference => {}
		err => panic!("unexpected error: {err:?}"),
	}
...
#[rstest]
fn create_merge_request_target_branch_only_local(_sandbox: TempDir, #[with(&_sandbox)] repos: Repos) -> Result {
...
fn signature_onelinear_deserialize_test() -> Result {
	let signature = SinglelineSignature {
		name: "test".to_string(),
