...
#[rstest]
fn lock_file_run_read_command_healthcheck_fails(sandbox: TempDir, #[with(&sandbox)] mut repo: Repo<TestCreds>) -> Result {
...
#[rstest]
fn lock_file_run_write_command_healthcheck_fails(sandbox: TempDir, #[with(&sandbox)] repo: Repo<TestCreds>) -> Result {
	let file_name = "test_file";
	std::fs::write(sandbox.path().join(file_name), "content")?;
	repo.add(file_name)?;
	let commit = repo.commit_debug()?;
