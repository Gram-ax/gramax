...
impl<C: Creds> Repo<'_, C> {
	...
	fn diff_without_merge_base(&self, opts: &DiffConfig) -> Result<git2::Diff<'_>> {
		...
		let diff = match &opts.compare {
			...
			DiffCompareOptions::Tree2Index { tree: tree_id } => {
				...
				self
					.0
					.diff_tree_to_index(base_commit.as_ref(), Some(&self.0.index()?), Some(&mut diff_opts))?
			}
		};
	...
	fn collect_diff_deltas(&self, diff: git2::Diff<'_>, used_merge_base: Option<Oid>) -> Result<DiffTree2TreeInfo> {
		...
		let mut file_callback = |diff: DiffDelta<'_>, _: f32| -> bool {
			...
			let blob = if status == Delta::Deleted {
				self.0.find_blob(diff.old_file().id()).ok()
			} else {
				self.0.find_blob(diff.new_file().id()).ok()
			};

			let maybe_lfs_pointer = blob.as_ref().and_then(|b| git2_lfs::Pointer::from_str_short(b.content()));
			...
			let file = DiffTree2TreeFile {
				...
				added: 0,
				deleted: 0,
				size: maybe_lfs_pointer.map(|p| p.size()).unwrap_or(0),
				is_lfs: maybe_lfs_pointer.is_some(),
			};
