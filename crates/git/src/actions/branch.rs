...
pub trait Branch {
	...
	fn branch_by_name<S: AsRef<str>>(&self, shorthand: S, branch_type: Option<BranchType>) -> Result<BranchEntry<'_>>;
...
impl<C: ActualCreds> RemoteBranch for Repo<'_, C> {
	...
	fn default_branch(&self) -> Result<Option<BranchEntry<'_>>> {
		let Ok(mut remote) = self.0.find_remote("origin") else { return Ok(None) };
		let Ok(()) = self.ensure_remote_connected(&mut remote, Direction::Fetch) else {
			warn!(target: TAG, "failed to connect to remote & get default branch");
			return Ok(None);
		};

		let branch_buf = match remote.default_branch() {
			Ok(b) => b,
			Err(e) => {
				warn!(target: TAG, "failed to get default branch: {e:?}");
				return Ok(None);
			}
		};
