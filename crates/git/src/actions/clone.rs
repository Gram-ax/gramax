...
impl<C: ActualCreds> Clone<C> for Repo<'_, C> {
	fn clone(creds: C, opts: CloneOptions, callback: RemoteProgressCallback) -> Result<()> {
		...
		if !allow_non_empty_dir && to.exists() && std::fs::read_dir(&to)?.next().is_some() {
			let err = git2::Error::new(ErrorCode::Exists, ErrorClass::Invalid, format!("dir `{}` is not empty", to.display()));
		...
		match repo.0.head().and_then(|h| h.peel_to_tree()) {
			Ok(tree) => {
				if !skip_lfs_pull {
					repo.pull_lfs_objects_by_tree(&tree, make_lfs_callback(callback.clone(), cancel_token.clone()), cancel_token.clone())?;
				} else {
					repo.set_lazy_lfs_enabled(true)?;
				}
