...
pub trait CreateRemoteTransferCallbacks<'c> {
  fn create_remote_transfer_callbacks(
    &'c self,
    cancel_token: CancelToken<'c>,
    on_progress: RemoteProgressCallback<'c>,
  ) -> RemoteCallbacks<'c>;
}
...
impl Display for RemoteProgress {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    match self {
      ...
      RemoteProgress::ChunkedTransfer { transfer, bytes, download_speed_bytes, .. } => {
        match transfer {
