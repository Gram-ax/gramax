...
pub fn ssl_callback(_cert: &cert::Cert, _host: &str) -> Result<CertificateCheckStatus, Error> {
	...
	if *DISABLE_SSL_CERT_CHECK.get_or_init(|| std::env::var("DISABLE_SSL_CERT_CHECK").is_ok()) {
		warn!(
			"SSL certificate validation bypassed (DISABLE_SSL_CERT_CHECK is set). This poses significant security risks like man-in-the-middle attacks"
		);
...
pub fn make_credentials_callback<C: Creds>(creds: &C) -> CredentialsCallback<'_> {
	...
	Box::new(
		move |url: &str, username: Option<&str>, allowed_type: CredentialType| -> Result<Cred, Error> {
			match allowed_type {
				allowed if allowed.contains(CredentialType::SSH_KEY) => {
					let Some(identity) = resolve_identity_from_config(url).or_else(|| identities.as_mut().and_then(|i| i.next())) else {
...
pub fn push_update_reference_callback(refname: &str, status: Option<&str>) -> std::result::Result<(), git2::Error> {
