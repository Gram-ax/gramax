...
impl<C: Creds> Repo<'_, C> {
	pub fn resolve_branch_entry<'b>(&'b self, branch: (git2::Branch<'b>, BranchType)) -> Result<BranchEntry<'b>> {
	...
	pub(crate) fn ensure_remote_has_postfix<'r>(&'r self, remote: git2::Remote<'r>) -> Result<git2::Remote<'r>> {
		let Some(url) = remote.url() else { return Ok(remote) };
		let Some(name) = remote.name() else { return Ok(remote) };
		...
		let remote = self.0.find_remote(name)?;
