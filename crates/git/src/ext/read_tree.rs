...
pub trait ReadTree {
	...
	fn read_dir_stats<P: AsRef<Path>>(&self, path: P) -> Result<Vec<DirStat>>;
...
impl<'r, C: Creds> RepoSelectTreeScope<'r, C> for Repo<'r, C> {
	...
	fn read_tree_commit(&self, oid: Oid) -> Result<RepoTreeScope<'_, 'r, C>> {
		Ok(RepoTreeScope {
			repo: self,
			tree: self.0.find_commit(oid)?.tree()?,
		})
	}
...
impl<C: Creds> ReadTree for RepoTreeScope<'_, '_, C> {
	...
	fn read_dir_stats<P: AsRef<Path>>(&self, path: P) -> Result<Vec<DirStat>> {
		self
			...
			.into_iter()
			.map(|item| self.stat(path.as_ref().join(&item.name)).map(|stat| DirStat { name: item.name, stat }))
			.collect()
	}
	...
	fn read_to_vec<P: AsRef<Path>>(&self, path: P) -> Result<Vec<u8>> {
		...
		let blob = match object.as_blob() {
			Some(blob) => blob,
			None => {
				let error_message = format!("the target object {} (at {}) is not a blob", object.id(), path.as_ref().display());
	...
	fn read_to_string<P: AsRef<Path>>(&self, path: P) -> Result<String> {
		let string =
			String::from_utf8(self.read_to_vec(path)?).map_err(|err| git2::Error::new(ErrorCode::Invalid, ErrorClass::Object, err.to_string()))?;
		Ok(string)
	}
}
