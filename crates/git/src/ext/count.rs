...
impl<C: Creds> Count for Repo<'_, C> {
	fn count_changed_files<P: AsRef<Path>>(&self, search_in: P) -> Result<UpstreamCountChangedFiles> {
		...
		opts
			...
			.pathspec(search_in.as_ref())
			.ignore_submodules(true);

		upstream_count_files.changed = self
			.0
			.diff_tree_to_index(Some(&head_tree), Some(&self.0.index()?), Some(&mut opts))?
			.deltas()
			.count();
		...
		let Ok(upstream_branch) = head_branch.upstream() else {
